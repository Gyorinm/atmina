...
class CartScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: items.isEmpty ? const _EmptyCartState() : Padding(
          ...
          child: Column(children: [
            Expanded(child: ListView.builder(itemCount: items.length, itemBuilder: (context, index) {
              ...
              return CartItemTile(item: item, canIncrement: item.quantity < item.product.stockQuantity,
                onIncrement: () { if (!ref.read(cartControllerProvider.notifier).increment(key)) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('لا يمكن تجاوز المخزون المتاح لمنتج "${item.product.name}"'))); },
            ...
            CartSummaryCard(totals: totals, onCheckout: (finalAmount) async {
              try {
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تم تسجيل البيع بمبلغ ${finalAmount.toCurrency()} وخصم الكميات من المخزون.')));
                Navigator.of(context).pop();
              } catch (error) {
                if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر تسجيل البيع: $error')));
              }
            }),
          ]),
        ),
