...
class CartScreen extends ConsumerWidget {
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('نقطة البيع')), body: SafeArea(child: items.isEmpty ? const _EmptyCartState() : Padding(padding: const EdgeInsets.fromLTRB(20, 10, 20, 20), child: Column(children: [
      Expanded(child: ListView.builder(itemCount: items.length, itemBuilder: (context, index) { final item = items[index]; final key = item.product.id ?? item.product.barcode.hashCode; return CartItemTile(item: item, canIncrement: item.quantity < item.product.stockQuantity, onIncrement: () { if (!ref.read(cartControllerProvider.notifier).increment(key)) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('لا يمكن تجاوز المخزون المتاح لمنتج "${item.product.name}"'))); }, onDecrement: () => ref.read(cartControllerProvider.notifier).decrement(key), onRemove: () => ref.read(cartControllerProvider.notifier).remove(key)); })),
      const SizedBox(height: 10),
      OutlinedButton.icon(onPressed: () async { try { final payload = OrderService(AppDatabase.instance).buildPayload(items.map((e) => (product: e.product, quantity: e.quantity)).toList()); final uri = Uri(scheme: 'atmina', host: 'order', path: '/${payload.encode()}'); await SharePlus.instance.share(ShareParams(text: 'طلبية Atmina #${payload.code}\nافتح هذا الرابط داخل تطبيق Atmina لإضافة الطلب:\n$uri', title: 'مشاركة الطلبية')); } catch (error) { if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر مشاركة الطلبية: $error'))); } }, icon: const Icon(Icons.share_outlined), label: const Text('نسخ ومشاركة رابط الطلبية')),
      const SizedBox(height: 10),
