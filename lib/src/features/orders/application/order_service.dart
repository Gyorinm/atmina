...
class OrderService {
  ...
  Future<int> saveSale({
    ...
    if (items.isEmpty) throw StateError('لا يمكن تسجيل بيع بدون منتجات.');
    if (received < total) throw StateError('المبلغ المستلم أقل من الإجمالي.');
  ...
  OrderPayload buildPayload(
    ...
    if (items.isEmpty) throw StateError('لا يمكن مشاركة سلة فارغة.');
    return OrderPayload(
      code: newCode(),
      createdAt: DateTime.now().toIso8601String(),
      note: note,
      items: items.map((e) => OrderPayloadItem(
  ...
  Future<int> importOrder(OrderPayload payload) {
    if (payload.items.isEmpty) throw StateError('الطلبية فارغة.');
