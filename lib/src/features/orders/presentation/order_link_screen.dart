...
class OrderLinkScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<OrderLinkScreen> createState() => _OrderLinkScreenState();
}
class _OrderLinkScreenState extends ConsumerState<OrderLinkScreen> {
  bool saving = false;
  @override Widget build(BuildContext context) {
    final total = widget.payload.items.fold<double>(0, (s, i) => s + i.unitPrice * i.quantity);
    return Directionality(textDirection: TextDirection.rtl, child: Scaffold(
      appBar: AppBar(title: const Text('طلبية مستلمة')),
      body: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
        Card(child: ListTile(title: Text('الطلب #${widget.payload.code}'), subtitle: Text('الإجمالي: ${total.toCurrency()}'))),
        const SizedBox(height: 12),
        Expanded(child: ListView(children: widget.payload.items.map((i) => ListTile(title: Text(i.name), subtitle: Text(i.barcode), trailing: Text('× ${i.quantity}'))).toList())),
        SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: saving ? null : _accept, icon: const Icon(Icons.add_shopping_cart), label: Text(saving ? 'جارٍ الاستلام...' : 'استلام الطلب وتجهيزه'))),
      ])),
    ));
  }
    ...
    try {
      ...
      for (final item in widget.payload.items) {
        ...
        for (var n = 0; n < item.quantity; n++) {
          if (!ref.read(cartControllerProvider.notifier).addProduct(product)) throw StateError('الكمية المطلوبة من ${item.name} تتجاوز المخزون المتاح.');
        }
      }
